...
> *The bootstrap distribution is a little centered near the population mean. Instead it is more centered near the sample mean. The bootstrap SD represents here is the standard deviation of the sample mean. From the data above the bootstrap SD shows the uncertainty based on the random sample.*
...
> *Yes the bootstrap distribution looks confident and narrow even if its centered far from the population mean. In this example, precision refers to how tightly clustered the bootstrap estimates are while accuracy refers to how close the estimate is to the true population mean. Here the estimate is precise but not accurate, because it is centered near the summer sample mean rather than the true annual mean. We have a sampling error shown here due from bias only sampling summer data. *
...
> *The random and stratified bootstrap distributions are both centered to the population mean, while the summer-only is centered away from it. This shows that the summer-only sample is biased, even though its bootstrap distribution looks narrow. The sampling design most accurate is the stratified due to it being well-centered and narrow. The most precise is the summer-only due to it being the most narrowest. This demonstrates that bootstrap reflects the sampling design: it measures uncertainty within the sample but cannot fix bias.*
...
*Yes the confidence interval contains the true population mean because it is about 35.5 and it is bounded between 34.8 and 36.9. Since the population mean lies inside the 95% bootstrap confidence interval, the estimate from the sample is consistent with the true value. This tells us that the uncertainty of the estimate is reasonably captured by the interval and since it is captured by the interval it suggests that the estimate is reliable and not biased.*
---
...
*The red vertical lines represent the range of values that are inside the 95% confidence interval.*
---
...
boot_coef <- replicate(B, {
  ...
  samp_rows <- df_regression %>% 
    slice_sample(n = nrow(df_regression), replace = TRUE)
...
*The slope confidence interval does not include zero. This implies there is a negative relationship between the predictor and the response variable.*
---
